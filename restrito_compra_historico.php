...
<?php
		$estatus="SELECT * FROM usuario where login='".$user."'";
		$permission=(mysqli_query($conexao,$estatus));
		$permission2=mysqli_fetch_assoc($permission);
		echo $permission2['admin'];
		if($permission2['admin']=='Sim'){
			$dados1 = mysqli_query($conexao,"SELECT * FROM compra where comprado='X' ORDER BY id_compra DESC")
		    or die(mysqli_error($conexao));
		}else{
			$dados1 = mysqli_query($conexao,"SELECT * FROM compra where comprado='X' and nome='".$user."' ORDER BY id_compra DESC")
		    or die(mysqli_error($conexao));
		}
 ?>
    ...
    $id_compra=$dados2['id_compra'];
	  echo "<tbody align='center'>";
		echo '<tr>';
		  ...
		  echo '<td>'.$dados2['preco_compra']*$dados2['quantidade'].'</td>';
