...
<hr/>
<h2>Seu carrinho</h2>
<p>Confira os seus pedidos, finalise ou esclua a compra</p><p>Seus produtos serão entregues em até 5 dias úteis</p>

<?php
 $dados1 = mysqli_query($conexao,"SELECT * FROM venda where comprado='' and cpf='".$_SESSION["cpf"]."' ORDER BY id_venda DESC")
    ...
    $id_venda=$dados2['id_venda'];
	  echo "<tbody align='center'>";
		echo '<tr>';
		  ...
		  echo '<td><a href="venda_carrinho_update.php?id_venda='.$id_venda.'"><button type="submit" class="btn btn-info btn-lg">Comprar</button></a>';
		  echo '<a href="venda_carrinho_apagar.php?id_venda='.$id_venda.'"><button type="submit" class="btn btn-danger btn-lg">Apagar</button></a></td>';
		  
		echo '</tr>';
		echo '</tbody>';
		$total+=$dados2['preco']*$dados2['quantidade'];
	}
